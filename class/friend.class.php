...
class friend extends DbConnection {
    ...
    function fetchStatus($arrArg = array()) {
        if ($arrArg) {
            ...
            $data ['tables'] = "friend f";
            $data ['columns'] = array (
                    "f.friendship_status as status" 
            );
            $data ['conditions'] = array (
                    "f.friend_id" => $arrArg ['id'],
                    "user_id" => $_SESSION ['id'] 
            );
            $result = $this->db->select ( $data );
            $row = $result->fetch ( PDO::FETCH_ASSOC );
    ...
    function sendRequest($arrArg) {
        if ($arrArg) {
            ...
            /*
             * $sql="insert into friend (user_id,friend_id,friendship_status,friend_request_date) "; $sql.=" values ('".$arrArg."','".$_SESSION['id']."','3',now())"; $result2=$ob->executeSQL($sql);
             */
            if ($result1 && $result2) {
    ...
    function getType($arrArg = array()) {
        if ($arrArg) {
            ...
            $row = $result->fetch ( PDO::FETCH_ASSOC );
            /*
             * $result=$ob->executeSQL($sql); $row=mysql_fetch_assoc($result);
             */
            return $row ['type'];
        }
    }
    /*
     * function removeFriend($arrArg) { if($arrArg) { $ob=new DbConnection(); $sql="update friend set friendship_status='4' where user_id='".$_SESSION['id']."' AND friend_id='".$arrArg."'"; //echo $sql; $result1=$ob->executeSQL($sql); $sql="update friend set friendship_status='4' where user_id='".$arrArg."' AND friend_id='".$_SESSION['id']."'"; $result2=$ob->executeSQL($sql); if($result1 && $result2) { return "Friend Removed."; } } }
     */
}
?>